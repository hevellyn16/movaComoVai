"use client";

import { useState, useEffect, useRef } from "react";
import { useRouter } from "next/navigation";
import { useEvents } from "@/hooks/useEvents";
import { useAuth } from "@/hooks/useAuth";
import { Event } from "@/types/event.types";
import { useTags } from "@/hooks/useTags";
import { TagResponseDTO } from "@/types/tag.types";

function formatDate(dateString: string) {
  const date = new Date(dateString);
  const months = [
    "Jan",
    "Fev",
    "Mar",
    "Abr",
    "Mai",
    "Jun",
    "Jul",
    "Ago",
    "Set",
    "Out",
    "Nov",
    "Dez",
  ];
  return `${date.getDate().toString().padStart(2, "0")} ${months[date.getMonth()]}, ${date.getFullYear()}`;
}

function formatTime(dateString: string) {
	return new Date(dateString).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" });
}

function inferStatus(startsAt: string, endsAt: string) {
  const now = Date.now();
  const start = new Date(startsAt).getTime();
  const end = new Date(endsAt).getTime();
  if (now < start)
    return {
      label: "Agendado",
      classes: "bg-blue-50 text-blue-600 border-blue-200/50",
    };
  if (now <= end)
    return {
      label: "Em andamento",
      classes: "bg-green-50 text-green-700 border-green-200/50",
    };
  return {
    label: "Encerrado",
    classes: "bg-gray-100 text-gray-500 border-gray-200/50",
  };
}

const ITEMS_PER_PAGE = 4;
const TAGS_PER_PAGE = 6;

export default function GestaoEventosPage() {
  const router = useRouter();

  // Hooks da API
  const {
    fetchAllEvents,
    deleteEvent,
    isLoading: isEventsLoading,
  } = useEvents();
  const { fetchAllTags, createTag, updateTag, deleteTag, tags, isLoading: isTagsLoading } = useTags();
  const { user, isLoading: isAuthLoading } = useAuth();

  // Estados Gerais
  const [activeTab, setActiveTab] = useState<"eventos" | "tags">("eventos");

  // Estados - Eventos
  const [events, setEvents] = useState<Event[]>([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [selectedTag, setSelectedTag] = useState<string | null>(null);
  const [currentPage, setCurrentPage] = useState(1);

  // Estados - Tags
  const [tagSearchTerm, setTagSearchTerm] = useState("");
  const [tagCurrentPage, setTagCurrentPage] = useState(1);
  const [isTagModalOpen, setIsTagModalOpen] = useState(false);
  const [editingTag, setEditingTag] = useState<TagResponseDTO | null>(null);
  const [tagFormName, setTagFormName] = useState("");

  const filterRef = useRef<HTMLDivElement>(null);

  // =======================================================================
  // CORREÇÃO DO LOOP AQUI:
  // =======================================================================
  useEffect(() => {
    const loadData = async () => {
      fetchAllTags();
      try {
        // Traz um lote grande de eventos para a gestão local
        const data = await fetchAllEvents(0, 100);
        setEvents(data.content);
      } catch (error) {
        console.error("Erro ao carregar eventos:", error);
      }
    };

    loadData();

    // Desabilitamos o aviso do linter para garantir que rode apenas 1 vez (On Mount)
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user]);

  // Bloqueio de acesso para usuários comuns
  useEffect(() => {
    if (!isAuthLoading && user && user.userType !== "ADMIN") {
      router.replace("/feed");
    }
  }, [user, isAuthLoading, router]);

  // Lógica para fechar o filtro ao clicar fora
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (filterRef.current && !filterRef.current.contains(e.target as Node)) {
        setIsFilterOpen(false);
      }
    }
    document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, []);

  // Ação de Deletar Evento
  const handleDeleteEvent = async (id: string) => {
    if (
      window.confirm(
        "Tem certeza que deseja excluir este evento permanentemente?",
      )
    ) {
      try {
        await deleteEvent(id);
        // Remove da lista local otimisticamente
        setEvents(events.filter((e) => e.id !== id));
      } catch (error) {
        alert("Não foi possível excluir o evento.");
      }
    }
  };

  // Ações de Tags
  const handleOpenCreateTag = () => {
    setEditingTag(null);
    setTagFormName("");
    setIsTagModalOpen(true);
  };

  const handleOpenEditTag = (tag: TagResponseDTO) => {
    setEditingTag(tag);
    setTagFormName(tag.tagName);
    setIsTagModalOpen(true);
  };

  const handleSaveTag = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!tagFormName.trim()) return;

    try {
      if (editingTag) {
        await updateTag(editingTag.id, { tagName: tagFormName.trim() });
      } else {
        await createTag({ tagName: tagFormName.trim() });
      }
      setIsTagModalOpen(false);
    } catch (error) {
      alert("Erro ao salvar a tag.");
    }
  };

  const handleDeleteTag = async (id: string) => {
    if (window.confirm("Tem certeza que deseja excluir esta tag? Eventos usando ela poderão ser afetados.")) {
      try {
        await deleteTag(id);
      } catch (error) {
        alert("Erro ao excluir a tag.");
      }
    }
  };


  // Aplicação dos filtros locais (Eventos)
  const filteredEvents = events.filter((event) => {
    const matchesSearch = event.eventName
      .toLowerCase()
      .includes(searchTerm.toLowerCase());
    const matchesTag = selectedTag
      ? event.tags?.some((t) => t.tagname === selectedTag)
      : true;
    return matchesSearch && matchesTag;
  });

  const totalPages = Math.ceil(filteredEvents.length / ITEMS_PER_PAGE);
  const startIndex = (currentPage - 1) * ITEMS_PER_PAGE;
  const currentEvents = filteredEvents.slice(startIndex, startIndex + ITEMS_PER_PAGE);

  // Aplicação dos filtros locais (Tags)
  const filteredTags = tags.filter((tag) =>
    tag.tagName.toLowerCase().includes(tagSearchTerm.toLowerCase())
  );
  const tagTotalPages = Math.ceil(filteredTags.length / TAGS_PER_PAGE);
  const tagStartIndex = (tagCurrentPage - 1) * TAGS_PER_PAGE;
  const currentTags = filteredTags.slice(tagStartIndex, tagStartIndex + TAGS_PER_PAGE);

  // Não renderiza nada enquanto verifica a autenticação ou se não for admin
  if (isAuthLoading || !user || user.userType !== "ADMIN") {
    return (
      <div className="w-[calc(100vw-224px)] min-h-screen bg-gray-50/50 flex items-center justify-center">
        <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-[#b91c1c]"></div>
      </div>
    );
  }

  return (
    <div className="w-[calc(100vw-224px)] min-h-screen bg-gray-50/50 p-8 font-sans antialiased relative">
      <div className="max-w-5xl mx-auto w-full">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
              Gestão
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              Painel administrativo para controle da plataforma
            </p>
          </div>
          <button
            onClick={() => router.push("/createEvent")}
            className="inline-flex items-center gap-2 bg-[#b91c1c] hover:bg-[#991b1b] text-white font-semibold px-5 py-2.5 rounded-xl transition-all duration-200 shadow-sm text-sm cursor-pointer"
          >
            + Criar Novo Evento
          </button>
        </div>

        {/* Tabs */}
        <div className="flex gap-4 border-b border-gray-200 mb-8">
          <button
            onClick={() => setActiveTab("eventos")}
            className={`pb-4 px-2 text-sm font-semibold transition-colors border-b-2 cursor-pointer ${
              activeTab === "eventos"
                ? "border-[#b91c1c] text-[#b91c1c]"
                : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
            }`}
          >
            Eventos
          </button>
          <button
            onClick={() => setActiveTab("tags")}
            className={`pb-4 px-2 text-sm font-semibold transition-colors border-b-2 cursor-pointer ${
              activeTab === "tags"
                ? "border-[#b91c1c] text-[#b91c1c]"
                : "border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300"
            }`}
          >
            Tags / Interesses
          </button>
        </div>

        {activeTab === "eventos" && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {/* Barra de ferramentas */}
            <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 justify-between items-center bg-white">
              <div className="relative w-full sm:max-w-xs">
                <span
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 material-symbols-outlined"
                  style={{ fontSize: 18 }}
                >
                  search
                </span>
                <input
                  type="text"
                  placeholder="Buscar por nome do evento..."
                  value={searchTerm}
                  onChange={(e) => {
                    setSearchTerm(e.target.value);
                    setCurrentPage(1);
                  }}
                  className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#b91c1c] transition-colors placeholder:text-gray-400"
                />
              </div>

              {/* Filtro por tag */}
              <div className="relative" ref={filterRef}>
                <button
                  onClick={() => setIsFilterOpen(!isFilterOpen)}
                  className={`inline-flex items-center gap-2 px-4 py-2 border rounded-xl text-sm font-medium transition-colors cursor-pointer ${
                    isFilterOpen || selectedTag
                      ? "bg-gray-50 border-gray-300 text-gray-900"
                      : "border-gray-200 text-gray-700 hover:bg-gray-50"
                  }`}
                >
                  <span
                    className="material-symbols-outlined"
                    style={{ fontSize: 16 }}
                  >
                    tune
                  </span>
                  Filtros
                  {selectedTag && (
                    <span className="w-2 h-2 rounded-full bg-[#b91c1c]" />
                  )}
                </button>

                {isFilterOpen && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-100 rounded-xl shadow-lg z-10 py-2 max-h-64 overflow-y-auto">
                    <div className="px-4 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                      Tags
                    </div>
                    <button
                      onClick={() => {
                        setSelectedTag(null);
                        setIsFilterOpen(false);
                        setCurrentPage(1);
                      }}
                      className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors cursor-pointer ${
                        selectedTag === null
                          ? "text-[#b91c1c] font-medium"
                          : "text-gray-600"
                      }`}
                    >
                      Todas
                    </button>
                    {tags.map((tag) => (
                      <button
                        key={tag.id}
                        onClick={() => {
                          setSelectedTag(tag.tagName);
                          setIsFilterOpen(false);
                          setCurrentPage(1);
                        }}
                        className={`w-full text-left px-4 py-2 text-sm hover:bg-gray-50 transition-colors cursor-pointer ${
                          selectedTag === tag.tagName
                            ? "text-[#b91c1c] font-medium"
                            : "text-gray-600"
                        }`}
                      >
                        {tag.tagName}
                      </button>
                    ))}
                  </div>
                )}
              </div>
            </div>

            {/* Tabela Eventos */}
            <div className="overflow-x-auto min-h-[300px] relative">
              {isEventsLoading && events.length === 0 ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 z-10">
                  <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#b91c1c]"></div>
                  <p className="text-sm text-gray-500 mt-3 font-medium">
                    Carregando eventos...
                  </p>
                </div>
              ) : null}

              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/70 border-b border-gray-100">
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider w-[40%]">
                      Evento
                    </th>
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider text-center">
                      Status
                    </th>
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider text-center">
                      Data
                    </th>
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider text-center">
                      Preço
                    </th>
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider text-center">
                      Ações
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {currentEvents.map((event) => {
                    const image = event.pictures?.[0]?.pictureUrl;
                    const status = event.endsAt
                      ? inferStatus(event.startsAt, event.endsAt)
                      : {
                          label: "Indefinido",
                          classes: "bg-gray-100 text-gray-500",
                        };

                    return (
                      <tr
                        key={event.id}
                        className="hover:bg-gray-50/50 transition-colors group"
                      >
                        {/* Nome + local */}
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 rounded-lg bg-gray-100 overflow-hidden shrink-0 border border-gray-200/60 shadow-sm">
                              {image ? (
                                <img
                                  src={image}
                                  alt={event.eventName}
                                  className="w-full h-full object-cover transition-transform group-hover:scale-105"
                                />
                              ) : (
                                <div className="w-full h-full bg-red-50 flex items-center justify-center font-bold text-[#b91c1c]">
                                  {event.eventName.charAt(0)}
                                </div>
                              )}
                            </div>
                            <div>
                              <h4 className="font-semibold text-gray-900 text-sm leading-snug">
                                {event.eventName}
                              </h4>
                              <p className="text-xs text-gray-400 mt-0.5">
                                {event.venue?.name ?? "Local não informado"}
                              </p>
                              {/* Tags */}
                              {event.tags && event.tags.length > 0 && (
                                <span className="inline-block mt-1 text-[10px] font-bold text-[#b91c1c] bg-red-50 px-1.5 py-0.5 rounded-full">
                                  {event.tags[0].tagname}
                                </span>
                              )}
                            </div>
                          </div>
                        </td>

                        {/* Status inferido via starts_at / ends_at */}
                        <td className="p-4 text-center">
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold border ${status.classes}`}
                          >
                            {status.label}
                          </span>
                        </td>

                        {/* Data */}
                        <td className="p-4 text-sm text-gray-600 font-medium text-center">
                          {formatDate(event.startsAt)}
                        </td>

                        {/* Preço */}
                        <td className="p-4 text-sm text-center">
                          {event.price === 0 ? (
                            <span className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-semibold bg-green-50 text-green-700 border border-green-200/50">
                              Gratuito
                            </span>
                          ) : (
                            <span className="font-medium text-gray-700">
                              {event.price.toLocaleString("pt-BR", {
                                style: "currency",
                                currency: "BRL",
                              })}
                            </span>
                          )}
                        </td>

                        {/* Ações */}
                        <td className="p-4 text-center">
                          <div className="flex items-center justify-center gap-1.5">
                            <button
                              onClick={() => router.push(`/events/${event.id}`)}
                              className="p-1.5 text-gray-400 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-all cursor-pointer"
                              title="Visualizar"
                            >
                              <span
                                className="material-symbols-outlined"
                                style={{ fontSize: 18 }}
                              >
                                visibility
                              </span>
                            </button>
                            <button
                              onClick={() =>
                                router.push(`/editEvent/${event.id}`)
                              }
                              className="p-1.5 text-gray-400 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-all cursor-pointer"
                              title="Editar"
                            >
                              <span
                                className="material-symbols-outlined"
                                style={{ fontSize: 18 }}
                              >
                                edit
                              </span>
                            </button>
                            <button
                              onClick={() => handleDeleteEvent(event.id)}
                              className="p-1.5 text-gray-400 hover:text-[#b91c1c] hover:bg-red-50 rounded-lg transition-all cursor-pointer"
                              title="Excluir"
                            >
                              <span
                                className="material-symbols-outlined"
                                style={{ fontSize: 18 }}
                              >
                                delete
                              </span>
                            </button>
                          </div>
                        </td>
                      </tr>
                    );
                  })}

                  {!isEventsLoading && filteredEvents.length === 0 && (
                    <tr>
                      <td colSpan={5} className="p-12 text-center">
                        <span
                          className="material-symbols-outlined text-gray-200 block mb-2"
                          style={{ fontSize: 40 }}
                        >
                          event_busy
                        </span>
                        <p className="text-sm text-gray-400">
                          Nenhum evento encontrado.
                        </p>
                      </td>
                    </tr>
                  )}
                </tbody>
              </table>
            </div>

            {/* Rodapé + paginação */}
            <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 bg-gray-50/40">
              <p className="text-xs text-gray-500 font-medium">
                Exibindo {filteredEvents.length === 0 ? 0 : startIndex + 1} a{" "}
                {Math.min(startIndex + ITEMS_PER_PAGE, filteredEvents.length)} de{" "}
                {filteredEvents.length} eventos
              </p>

              {totalPages > 1 && (
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))}
                    disabled={currentPage === 1}
                    className="p-1 rounded-lg border border-gray-200 hover:bg-white text-gray-500 disabled:opacity-50 transition-colors cursor-pointer"
                  >
                    ‹
                  </button>
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                    (page) => (
                      <button
                        key={page}
                        onClick={() => setCurrentPage(page)}
                        className={`w-7 h-7 inline-flex items-center justify-center text-xs font-bold rounded-lg transition-colors cursor-pointer border ${
                          currentPage === page
                            ? "bg-[#b91c1c] text-white border-[#b91c1c]"
                            : "text-gray-600 hover:bg-white border-transparent hover:border-gray-200"
                        }`}
                      >
                        {page}
                      </button>
                    ),
                  )}
                  <button
                    onClick={() =>
                      setCurrentPage((p) => Math.min(p + 1, totalPages))
                    }
                    disabled={currentPage === totalPages}
                    className="p-1 rounded-lg border border-gray-200 hover:bg-white text-gray-500 disabled:opacity-50 transition-colors cursor-pointer"
                  >
                    ›
                  </button>
                </div>
              )}
            </div>
          </div>
        )}

        {activeTab === "tags" && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            {/* Container Tags */}
            {/* Barra de ferramentas */}
            <div className="p-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 justify-between items-center bg-white">
              <div className="relative w-full sm:max-w-xs">
                <span
                  className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 material-symbols-outlined"
                  style={{ fontSize: 18 }}
                >
                  search
                </span>
                <input
                  type="text"
                  placeholder="Buscar tag..."
                  value={tagSearchTerm}
                  onChange={(e) => {
                    setTagSearchTerm(e.target.value);
                    setTagCurrentPage(1);
                  }}
                  className="w-full pl-9 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#b91c1c] transition-colors placeholder:text-gray-400"
                />
              </div>

              <button
                onClick={handleOpenCreateTag}
                className="inline-flex items-center gap-2 bg-gray-900 hover:bg-gray-800 text-white font-semibold px-4 py-2 rounded-xl transition-all shadow-sm text-sm cursor-pointer"
              >
                + Nova Tag
              </button>
            </div>

            {/* Tabela Tags */}
            <div className="overflow-x-auto min-h-[300px] relative">
              {isTagsLoading && tags.length === 0 ? (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/80 z-10">
                  <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-[#b91c1c]"></div>
                  <p className="text-sm text-gray-500 mt-3 font-medium">
                    Carregando tags...
                  </p>
                </div>
              ) : null}

              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/70 border-b border-gray-100">
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider">
                      Nome da Tag
                    </th>
                    <th className="p-4 text-xs font-semibold uppercase text-gray-500 tracking-wider text-right w-32">
                      Ações
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {currentTags.map((tag) => (
                    <tr
                      key={tag.id}
                      className="hover:bg-gray-50/50 transition-colors group"
                    >
                      <td className="p-4">
                        <span className="inline-block font-bold text-[#b91c1c] bg-red-50 px-3 py-1 rounded-full text-sm">
                          {tag.tagName}
                        </span>
                      </td>
                      <td className="p-4 text-right">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            onClick={() => handleOpenEditTag(tag)}
                            className="p-1.5 text-gray-400 hover:text-yellow-600 hover:bg-yellow-50 rounded-lg transition-all cursor-pointer"
                            title="Editar"
                          >
                            <span className="material-symbols-outlined" style={{ fontSize: 18 }}>edit</span>
                          </button>
                          <button
                            onClick={() => handleDeleteTag(tag.id)}
                            className="p-1.5 text-gray-400 hover:text-[#b91c1c] hover:bg-red-50 rounded-lg transition-all cursor-pointer"
                            title="Excluir"
                          >
                            <span className="material-symbols-outlined" style={{ fontSize: 18 }}>delete</span>
                          </button>
                        </div>
                      </td>
                    </tr>
                  ))}

                  {!isTagsLoading && filteredTags.length === 0 && (
                    <tr>
                      <td colSpan={2} className="p-12 text-center">
                        <span
                          className="material-symbols-outlined text-gray-200 block mb-2"
                          style={{ fontSize: 40 }}
                        >
                          sell
                        </span>
                        <p className="text-sm text-gray-400">
                          Nenhuma tag encontrada.
                        </p>
                      </td>
                    </tr>
                  )}
                </tbody>
              </table>
            </div>

            {/* Rodapé + paginação Tags */}
            <div className="p-4 border-t border-gray-100 flex flex-col sm:flex-row justify-between items-center gap-3 bg-gray-50/40">
              <p className="text-xs text-gray-500 font-medium">
                Exibindo {filteredTags.length === 0 ? 0 : tagStartIndex + 1} a{" "}
                {Math.min(tagStartIndex + TAGS_PER_PAGE, filteredTags.length)} de{" "}
                {filteredTags.length} tags
              </p>

              {tagTotalPages > 1 && (
                <div className="flex items-center gap-1.5">
                  <button
                    onClick={() => setTagCurrentPage((p) => Math.max(p - 1, 1))}
                    disabled={tagCurrentPage === 1}
                    className="p-1 rounded-lg border border-gray-200 hover:bg-white text-gray-500 disabled:opacity-50 transition-colors cursor-pointer"
                  >
                    ‹
                  </button>
                  {Array.from({ length: tagTotalPages }, (_, i) => i + 1).map(
                    (page) => (
                      <button
                        key={page}
                        onClick={() => setTagCurrentPage(page)}
                        className={`w-7 h-7 inline-flex items-center justify-center text-xs font-bold rounded-lg transition-colors cursor-pointer border ${
                          tagCurrentPage === page
                            ? "bg-[#b91c1c] text-white border-[#b91c1c]"
                            : "text-gray-600 hover:bg-white border-transparent hover:border-gray-200"
                        }`}
                      >
                        {page}
                      </button>
                    ),
                  )}
                  <button
                    onClick={() =>
                      setTagCurrentPage((p) => Math.min(p + 1, tagTotalPages))
                    }
                    disabled={tagCurrentPage === tagTotalPages}
                    className="p-1 rounded-lg border border-gray-200 hover:bg-white text-gray-500 disabled:opacity-50 transition-colors cursor-pointer"
                  >
                    ›
                  </button>
                </div>
              )}
            </div>
          </div>
        )}
      </div>

      {/* Modal Criar/Editar Tag */}
      {isTagModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-xl p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">
              {editingTag ? "Editar Tag" : "Nova Tag"}
            </h2>
            <form onSubmit={handleSaveTag}>
              <div className="mb-5">
                <label className="block text-sm font-semibold text-gray-700 mb-1">
                  Nome da Tag <span className="text-[#b91c1c]">*</span>
                </label>
                <input
                  type="text"
                  required
                  autoFocus
                  placeholder="Ex: Rock"
                  value={tagFormName}
                  onChange={(e) => setTagFormName(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#b91c1c] transition-colors"
                />
              </div>
              <div className="flex gap-3 justify-end">
                <button
                  type="button"
                  onClick={() => setIsTagModalOpen(false)}
                  className="px-4 py-2 text-sm font-semibold text-gray-600 hover:bg-gray-100 rounded-xl transition-colors cursor-pointer"
                >
                  Cancelar
                </button>
                <button
                  type="submit"
                  disabled={isTagsLoading}
                  className="px-4 py-2 text-sm font-semibold text-white bg-[#b91c1c] hover:bg-[#991b1b] rounded-xl transition-colors shadow-sm disabled:opacity-50 cursor-pointer"
                >
                  {isTagsLoading ? "Salvando..." : "Salvar"}
                </button>
              </div>
            </form>
          </div>
        </div>
      )}
    </div>
  );
}
